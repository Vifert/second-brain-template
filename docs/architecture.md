...
<picture>
  ...
  <img alt="How the second brain works: capture, the wiki graph, the build, the index layer and the query ladder, with the guards beneath and what the agent loads, and when" src="assets/architecture.png">
</picture>
...
through the index. A sixth zone beneath shows what the agent itself loads,
and when.

Below is the same drawing as text — the transcription the vault itself
retrieves from, in `wiki/tooling/second-brain-architecture-figure.md` — so
every box, arrow and label is searchable.
