...
> the defect discipline with its eighty-nine mechanically guarded defects,
...
- **Before doing anything else in a new session, read HANDOFF.md** in this same
  folder. Claude Code carries no memory between sessions, so HANDOFF.md is the
...
Some rules live in `.claude/rules/` and load only when a file their `paths:`
match is read. Before writing where one applies, read it if it has not loaded
this session. Each is backed by the builder, so a slip fails the build.
...
| `.claude/rules/tooling.md` | `tools/**` | the tools, the hooks that enforce three traps, Windows file locks |
...
- `vault-excalidraw` — any drawing request — and `vault-tailor` — fitting the
  ...
  rounds; run once at setup, and whenever I ask — the vault skills Claude may
  start itself.
- `/vault-compile`, `/vault-audit`, `/vault-deep-audit`, `/vault-handoff` —
  {{OWNER_NAME}}'s to start (§ Compile and Audit, § Session Handoff).
- `/obsidian:defuddle` for any URL, before WebFetch. The Obsidian CLI is
...
- **A correction is swept, then fenced.** When I correct a fact, grep the whole
  ...
  employer survived in one unswept node for three days.
- **A fence proves itself both ways** (D50). Every `_forbidden` entry carries
  `catches` — the wrong claims it must match — and `passes` — the nearby true
  claims it must let through; the build replays both. Fence the error, not the
  topic: the first such fence forbade "is his manager" when only the employer
  was wrong — and a later, true statement about that same person's role
  would have failed. **When a new fact touches a fenced claim, add it to
  `passes` first.**
...
I type, and all four carry
`disable-model-invocation: true`, so Claude Code itself refuses to start them
for you.
...
- **If I ask in plain words** ("compile it", "audit the vault"), ask me to
  ...
  `/vault-audit`).
- **Why skills, not only a rule**: the procedures live in the skill bodies,
  which load only when invoked, so this file is lighter every session and
  nothing but my command can start them.
...
3. **Prevent the class**, preferring in this order: a **mechanical guard**
   ...
   heuristics, a hook for a trap in how Claude works) → a **rule** in the right
...
- **ripgrep (`rg`) is available**; the Grep tool is ripgrep-backed. Either is
  fine for index lookups.
