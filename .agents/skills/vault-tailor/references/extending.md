...
| a rule every session must follow | a line in `AGENTS.md` | its tokens in every session and every subagent | `AGENTS.md` § People |
...
- **Could code check it?** Then it is a guard, and the rule text only explains
  it. "A rule without a guard is a hope" (`AGENTS.md` § Defect Discipline).
- **Does every session need it?** If not, it does not belong in `AGENTS.md`.
  The core is paid on every turn by every session and every subagent.

## Keeping queries cheap

Whatever you add must stay inside the query ladder (`AGENTS.md` § Query
...
and undo the split. Add a line for it to the table in `AGENTS.md` § Rules That
...
rows start at **D200**. A hook (`tools/hooks/`, registered in
...
The vault runs under any coding agent. `.claude/` is the one source; for
agents that do not read it, `node tools/agents-sync.js` generates the rest.
Whatever you add, add it to `.claude/`, run the sync, and tell the owner what
their agent enforces and what it only reads as a rule.

| In `.claude/` | What it does | Codex | Gemini CLI and others |
| --- | --- | --- | --- |
| `CLAUDE.md` → `AGENTS.md` | rules every session reads | reads `AGENTS.md` | reads `AGENTS.md` (Gemini: set `context.fileName`) |
| `.claude/rules/` with `paths:` | rules that load only for certain files | a rule: read the file before writing where it applies | the same |
| Skills (`SKILL.md`) | procedures loaded on use | `.agents/skills/`, generated | `.agents/skills/` where the agent reads it |
| `disable-model-invocation` | only the owner starts a procedure | `agents/openai.yaml`, `allow_implicit_invocation: false`, generated | the skill's description and the manual's rule — a rule without a guard |
| Subagents (`.claude/agents/`) | work in a separate context | `.codex/agents/*.toml`, read-only sandbox, generated | the agent does the step itself |
| Hooks (`.claude/settings.json`) | block a command or a file before harm | `.codex/hooks.json`, generated; runs once the project and each hook are trusted | a written rule, with the builder and the self-test as the backstop |
| The tools in `tools/` | build, validate, test | unchanged — plain Node.js | unchanged |
