...
### [[wiki/journal/_index|Journal]] · 2 nodes
...
- [[mira-identity|Who Mira Okafor is]]
- [[vault-capture-protocol|How to dump things in here]]
- [[second-brain-architecture|How this vault works]]

## Retrieval Layer

- `wiki/_index.tsv` — **41 rows**: path, title, topic, kind, status, aliases, tags, summary. Route here first.
- `wiki/_cards.tsv` — **41 rows**: path, title, and the full Key Takeaways block. Pull exactly one row.
- `wiki/_sections.tsv` — **291 rows**: path, heading, start line, end line, gist. Gives exact `sed` ranges with no discovery read.
- `wiki/_links.tsv` — **245 edges**: source, target. Neighbourhood expansion without reading articles.
- `wiki/_mentions.tsv` — **42 rows**: person, date, log, line range, snippet. Every dated log entry that names or links someone.
