---
title: "Tidewater: Why Postgres, Not DynamoDB — Full Text"
summary: The full text of the one-page note Mira sent the platform team after the kickoff, arguing Postgres for Tidewater, as written.
aliases: [the Postgres note, the one-page note, Postgres versus DynamoDB note, why Postgres not DynamoDB, the note to the platform team, the storage note]
topic: projects
kind: detail
verbatim: true
source: raw/notes/tidewater-storage-choice.md
tags: [org/harbor-analytics, tech/postgres, tech/dynamodb, field/data-engineering, form/full-text]
created: 2026-09-23
updated: 2026-09-23
---
...
- **The one-page Postgres note is reproduced here in full**, as Mira Okafor wrote it for the platform team after the Tidewater kickoff of 10 March 2026.
- **The note gives four reasons for Postgres**: the dashboard needs joins, the questions are ad-hoc filters, the data is small, and the team already writes SQL.
- **The note answers DynamoDB's three advantages rather than ignoring them** — scaling, no server to patch, and being the platform default — and says why none of them decides this case.
- **The note names what would change the answer**: thousands of calls a second with fixed access patterns, or the ops team wanting events as they happen.
- **The note's mitigations are an index-as-you-build habit, a read-only role and a schema guide**, so nobody writes to Tidewater's tables but Tidewater.

## Source Text

Reproduced from `raw/notes/tidewater-storage-choice.md`. The heading below is the
...
- [[tidewater-postgres-over-dynamodb|the storage decision]] — the distilled version, with what has happened since.
- [[tidewater-dashboard|Tidewater]] — the project the note is about.
- [[tidewater-data-model|the Tidewater data model]] — the tables and indexes the note argues for.
